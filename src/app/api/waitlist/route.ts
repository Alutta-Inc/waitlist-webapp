...
// The client address, the origin rule, Turnstile and the brake are shared with
// the researcher removal door; see lib/public-door.ts for why each is there.
// The budget below is this route's own.
const brake = createBrake({ windowMs: 10 * 60 * 1000, maxRequests: 60, maxFailures: 15 });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      turnstileValid = await verifyTurnstileToken(String(turnstileToken || ""), ip, TURNSTILE_ACTION);
    } catch (e) {
