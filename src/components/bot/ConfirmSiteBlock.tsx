...
const copy: ConfirmCopy = {
  ...
  headingFor: (data) => (typeof data.domain === "string" && data.domain ? `Stop AluttaBot visiting ${data.domain}.` : null),
  alreadyBody: (data) => {
    const domain = typeof data.domain === "string" && data.domain ? data.domain : "your website";
    return `You confirmed this already. AluttaBot does not visit ${domain}.`;
  },
...
export default function ConfirmSiteBlock() {
  return <ConfirmLink endpoint="/api/bot/block/confirm" checkEndpoint="/api/bot/block/check" copy={copy} successKey="blocked" />;
}
